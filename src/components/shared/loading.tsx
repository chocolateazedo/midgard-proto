...
export function CardSkeleton({ className }: { className?: string }) {
  return (
    <div
      className={cn(
        "rounded-lg border border-slate-200/60 bg-white p-6 space-y-3",
        className
      )}
    >
...
export function TableSkeleton({
  ...
  return (
    <div
      className={cn(
        "rounded-md border border-slate-200/60 overflow-hidden",
        className
      )}
    >
      {/* Header */}
      ...
      {Array.from({ length: rows }).map((_, rowIdx) => (
        <div
          key={rowIdx}
          className="border-b border-slate-200/60 last:border-0 px-4 py-3"
        >
...
function ChartSkeleton({ className }: { className?: string }) {
  return (
    <div
      className={cn(
        "rounded-lg border border-slate-200/60 bg-white p-6",
        className
      )}
    >
      <Skeleton className="h-5 w-48 bg-slate-100 mb-1" />
      <Skeleton className="h-3 w-64 bg-slate-100 mb-6" />
